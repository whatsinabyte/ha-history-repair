"""MariaDB/MySQL implementation of DatabaseAdapter.

Every SQL string in the project lives in this module. The queries target
recorder schema 28 and above, where states.entity_id has been replaced by a
metadata_id foreign key into states_meta.
"""
...
# MySQL error codes translated into domain errors.
...
CREATE TABLE IF NOT EXISTS state_corrections (
  ...
  sensor_type           ENUM('measurement','counter','unknown')
  ...
  state_ts              DOUBLE NOT NULL,
  state_dt              DATETIME DEFAULT NULL,

  -- Originals captured before the change. This phase fills only the states
  -- column; the statistics columns are reserved for the phase that corrects
  -- statistics_short_term and statistics.
  orig_state_value      VARCHAR(255),
  orig_sst_mean         DOUBLE,
  orig_sst_min          DOUBLE,
  orig_sst_max          DOUBLE,
  orig_sst_state        DOUBLE,
  orig_sst_sum          DOUBLE,
  orig_stat_mean        DOUBLE,
  orig_stat_min         DOUBLE,
  orig_stat_max         DOUBLE,
  orig_stat_state       DOUBLE,
  orig_stat_sum         DOUBLE,

  corrected_value       VARCHAR(255) NOT NULL,

  sum_delta_applied     DOUBLE DEFAULT NULL,
  cascade_rows_updated  INT DEFAULT NULL,

  -- This phase corrects states only, so the flag stays 0 and a later phase
  -- can find every correction whose statistics still need recomputing.
  stats_corrected       TINYINT(1) NOT NULL DEFAULT 0,

  quality               ENUM('bad_comm','out_of_range','spike','frozen',
                             'uncertain') NOT NULL DEFAULT 'uncertain',
  ...
  dismissed_by          VARCHAR(255) DEFAULT NULL,

  KEY idx_entity_ts (entity_id, state_ts),
  KEY idx_state_id (state_id),
  KEY idx_active (entity_id, restored_at)
...
_AUDIT_TABLE_MIGRATIONS = (
    "ALTER TABLE state_corrections ADD COLUMN IF NOT EXISTS "
    + "dismissed_at DATETIME DEFAULT NULL",
    "ALTER TABLE state_corrections ADD COLUMN IF NOT EXISTS "
    + "dismissed_by VARCHAR(255) DEFAULT NULL",
)
...
class MariaDBAdapter(DatabaseAdapter):
    ...
    def _connect(self) -> pymysql.connections.Connection:
        try:
            return pymysql.connect(
                ...
                read_timeout=60,
                write_timeout=60,
                # Without this, MySQL's protocol reports rows *changed* by an
                # UPDATE, not rows *matched* — 0 whenever a correction's new
                # value happens to equal the row's current one. Every
                # `cur.rowcount != 1` check in this adapter (guarding against
                # a row disappearing mid-transaction) would then misfire on
                # that same, otherwise ordinary, no-op correction.
                client_flag=CLIENT.FOUND_ROWS,
                init_command=(
                    f"SET SESSION innodb_lock_wait_timeout = {self._config.lock_wait_timeout}"
                ),
            )
    ...
    def _transaction(self) -> Iterator[Any]:
        ...
        try:
            ...
            if code in (_ER_LOCK_WAIT_TIMEOUT, _ER_LOCK_DEADLOCK):
                raise LockTimeout(
    ...
    def check_health(self) -> HealthReport:
        ...
        try:
            with self._cursor() as cur:
                ...
                row = cur.fetchone()
                server_version = row["v"] if row else None
                ...
                if not can_update:
                    errors.append(
                        "The database user lacks the UPDATE privilege on "
                        f"'{self._config.name}'. Grant it in the MariaDB add-on "
                        "configuration."
                    )
                ...
                audit_ready = bool(cur.fetchone()["n"])
    ...
    def _has_update_privilege(cur: Any) -> bool:
        ...
        for row in cur.fetchall():
            ...
            if "ALL PRIVILEGES" in head or re.search(r"\bUPDATE\b", head):
                return True
        return False
        ...
        if self._stats_meta_columns is None:
            with self._cursor() as cur:
                cur.execute(
                    ...
                    "WHERE table_schema = %s AND table_name = 'statistics_meta'",
                    (self._config.name,),
                )
    ...
    def _entity_sort_expr(sort: str, sort_dir: str) -> str:
        ...
        if sort == "last_updated":
            # NULLS-last regardless of direction, via its own fixed-direction
            # flag column: an entity with no states at all should not jump to
            # the top of a "most recently updated" sort just because NULL
            # sorts first by default.
    ...
    def _entity_filter_clause(
        ...
        params: list[Any] = []
        if search:
            clauses.append("sm.entity_id LIKE %s ESCAPE '\\\\'")
    ...
    def _list_entities_sql(self, where: str, order_by: str, correction_expr: str) -> str:
        ...
        return f"""
            SELECT
              ...
              JSON_UNQUOTE(JSON_EXTRACT(sa.shared_attrs, '$.friendly_name')) AS friendly_name
            FROM (
    ...
    def list_entities(
        ...
        # COLLATE is required here, found the hard way against a real
        # installation: state_corrections is this add-on's own table, created
        # with whatever utf8mb4 collation the server defaulted to at CREATE
        # TABLE time (AUDIT_TABLE_DDL specifies the charset but not a
        # collation), which does not necessarily match whatever collation
        # Home Assistant's own SQLAlchemy schema gave states_meta.entity_id.
        # Comparing two columns with different implicit collations is a hard
        # MySQL/MariaDB error (1267), not a mismatch this adapter can control
        # by fixing one side's table definition — the *recorder's* collation
        # varies across Home Assistant versions and cannot be relied on to
        # match whatever this add-on's table happened to get. An explicit
        # COLLATE on either side of the comparison wins over both implicit
        # ones and resolves it regardless of what either actually is.
        correction_expr = (
            "(SELECT COUNT(*) FROM state_corrections sc "
            "WHERE sc.entity_id = sm.entity_id COLLATE utf8mb4_general_ci "
            "AND sc.restored_at IS NULL)"
        )
        ...
        with self._cursor() as cur:
            try:
                ...
                # audit table yet means no corrections yet either.
    ...
    def _row_to_entity(cls, row: dict[str, Any], counts: dict[str, int]) -> Entity:
        return Entity(
            ...
            friendly_name=row.get("friendly_name") or None,
    ...
    @staticmethod
    def _is_missing_audit_table(err: pymysql.Error) -> bool:
        ...
        """
        return bool(err.args) and err.args[0] == _ER_NO_SUCH_TABLE

    def _correction_counts(self, entity_ids: list[str]) -> dict[str, int]:
        if not entity_ids:
            return {}
        # nosec B608 - placeholders is a run of "%s" markers sized to the
        # list; every entity id is bound as a parameter.
        placeholders = ", ".join(["%s"] * len(entity_ids))
        with self._cursor() as cur:
            try:
                cur.execute(
                    "SELECT entity_id, COUNT(*) AS n FROM state_corrections "
                    f"WHERE restored_at IS NULL AND entity_id IN ({placeholders}) "  # nosec B608  # nosec B608
                    "GROUP BY entity_id",
                    entity_ids,
                )
            except pymysql.Error as err:
    ...
    def restore_correction(self, correction_id: int, restored_by: str) -> Correction:
        with self._transaction() as cur:
            ...
            cur.execute(
                "UPDATE state_corrections SET restored_at = UTC_TIMESTAMP(), "
    ...
    def find_orphaned_corrections(self) -> list[Correction]:
        ...
        #
        # s.state != c.corrected_value needs the same COLLATE fix as
        # list_entities's correction_count subquery, and for the same
        # reason: state_corrections is this add-on's own table, created with
        # whatever utf8mb4 collation the server defaulted to, which does not
        # necessarily match whatever collation Home Assistant's own recorder
        # schema gave states.state. See the comment there for the full
        # explanation — found against a real installation where the two
        # differed.
        with self._cursor() as cur:
            cur.execute(
                ...
                "OR s.state != c.corrected_value COLLATE utf8mb4_general_ci) "
    ...
    def dismiss_correction(self, correction_id: int, dismissed_by: str) -> Correction:
        with self._transaction() as cur:
            ...
            cur.execute(
                "UPDATE state_corrections SET dismissed_at = UTC_TIMESTAMP(), "
